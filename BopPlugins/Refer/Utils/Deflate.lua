...
import "BopPlugins.Refer.Utils.HuffmanEncoding"; --!*!
import "BopPlugins.Refer.Utils.Log"; --!*!

local table_insert = table.insert;
local HuffmanEncoding = BopPlugins.Refer.Utils.HuffmanEncoding; --!*!
	...
	for _,datum in ipairs( lz77Data ) do
		if type(datum) == "table" then
			...
			if literalsLengthsCodesLookupTable[datum][2] > 15 then
				BopPlugins.Refer.Utils.Log.Error( "Deflate", string.format("Litlen code too long (%d)", literalsLengthsCodesLookupTable[datum][2] ) ); --!*!
			end
		end
	end
	return true;
...
local EncodeLZ77Data = function( outStream, lz77Data )
	...
	for _,length in ipairs( reorderedPackCodeLengths ) do
		if length > 7 then
			BopPlugins.Refer.Utils.Log.Error( "Deflate", string.format("pack code too long (%d)", length ) ); --!*!
		end
...
Deflate = function( data, outStream, strategy )
	...
	if lz77Data == nil then
		BopPlugins.Refer.Utils.Log.Error( "Deflate", "An error occured while compressing data" ); --!*!
		return false;
	end
