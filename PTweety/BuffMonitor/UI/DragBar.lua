...
local deltaButtonAvailable = pcall( function( ) import "PTweety.BuffMonitor.UI.DeltaButton"; end );

local defaultDragBarResources = {
	  Bar = {
				  Normal="PTweety/BuffMonitor/Resources/DragBarBar.tga"
				, Dragged="PTweety/BuffMonitor/Resources/DragBarBar_Dragged.tga"
			}
	, Corner = {
				  Normal="PTweety/BuffMonitor/Resources/DragBarCorner.tga"
				, Dragged="PTweety/BuffMonitor/Resources/DragBarCorner_Dragged.tga"
			   }
	, Hover = {
				  TopLeft="PTweety/BuffMonitor/Resources/DragBarHoverBox_TopLeft.tga"
				, Top="PTweety/BuffMonitor/Resources/DragBarHoverBox_Top.tga"
				, TopRight="PTweety/BuffMonitor/Resources/DragBarHoverBox_TopRight.tga"
				, Left="PTweety/BuffMonitor/Resources/DragBarHoverBox_Left.tga"
				, Right="PTweety/BuffMonitor/Resources/DragBarHoverBox_Right.tga"
				, BottomLeft="PTweety/BuffMonitor/Resources/DragBarHoverBox_BottomLeft.tga"
				, Bottom="PTweety/BuffMonitor/Resources/DragBarHoverBox_Bottom.tga"
				, BottomRight="PTweety/BuffMonitor/Resources/DragBarHoverBox_BottomRight.tga"
			  }
};

local defaultRotatorResources = {
	  Normal="PTweety/BuffMonitor/Resources/rotate_button_normal.tga"
	, Hover="PTweety/BuffMonitor/Resources/rotate_button_rollover.tga"
	, Pressed="PTweety/BuffMonitor/Resources/rotate_button_pressed.tga"
	, Disabled="PTweety/BuffMonitor/Resources/rotate_button_ghosted.tga"
};
...
function DragBar:Constructor( ctlAttachTo, sName, bBarOnTop, tResources )
	...
	self.MouseMove = function( sender, args )
		if ( self:IsBeingDragged() ) then
			...
			local x = math.min( math.max( left + ( args.X - self.dragStartX ), 0 ), Turbine.UI.Display:GetWidth() - self.wndTarget:GetWidth() );
			local y = math.min( math.max( top + ( args.Y - self.dragStartY ), 0 ), Turbine.UI.Display:GetHeight() - self.wndTarget:GetHeight());

			if type( self.wndTarget.ValidatePosition ) == "function" then
				self.wndTarget:ValidatePosition( x, y );
			else
...
function DragBar:SetRotator( bEnabled, tResources, bfZoneTest )
	if deltaButtonAvailable then
		if bEnabled then
			...
			self.Rotator = PTweety.BuffMonitor.UI.DeltaButton( tResources, bfZoneTest, self );
			self.Rotator:SetZOrder( self:GetZOrder() + 1 );

			self.Rotator.MouseEnter = function( sender, args )
				self:CreateHoverBox();
			end

			self.Rotator.MouseLeave = function( sender, args )
				if not self:IsBeingDragged() then
					self:DestroyHoverBox();
				end
			end
